...
- [x] Deploy scripts to Raspberry Pi
  - [x] Create setup directory on Pi: `ssh pi@raspberrypi.local "mkdir -p ~/setup"`
  - [x] Copy entire setup directory: `scp -r setup/raspberry-pi/* pi@raspberrypi.local:~/setup/`
  - [x] Verify files transferred: `ssh pi@raspberrypi.local "ls -la ~/setup/scripts/"`

- [x] Enable I2C interface (AC: 1)
  - [x] Used automated script `enable-interfaces.sh` (added `dtparam=i2c_arm=on` to `/boot/firmware/config.txt`)
  - [x] Verify I2C enabled: `ls /dev/i2c*` shows `/dev/i2c-1` ✓

- [x] Enable 1-Wire interface (AC: 2)
  - [x] Used automated script `enable-interfaces.sh` (added `dtoverlay=w1-gpio` to `/boot/firmware/config.txt`)
  - [x] Verify 1-Wire enabled: `ls /sys/bus/w1/devices/` exists ✓

- [x] Install I2C tools (AC: 3)
  - [x] Automated script installed i2c-tools package (version 4.4)
  - [x] Verify installation: `/usr/sbin/i2cdetect -V` shows version 4.4 ✓

- [x] Test I2C bus scanning (AC: 4)
  - [x] Scan I2C bus 1: `/usr/sbin/i2cdetect -y 1` ran successfully ✓
  - [x] Grid displayed with "--" in all cells (no sensors connected yet - expected)
  - [x] Expected I2C addresses for future sensor testing:
    - 0x36: STEMMA Soil Sensor (Story 1.4)
    - 0x23: BH1750 Light Sensor (Story 1.4)
    - 0x38: DHT20 Temp/Humidity Sensor (Story 1.4)

- [x] Verify 1-Wire device tree overlay (AC: 5)
  - [x] Check config file: `cat /boot/firmware/config.txt | grep w1-gpio` ✓
  - [x] Output contains: `dtoverlay=w1-gpio` ✓
  - [x] Reboot performed after config changes
  - [x] 1-Wire modules loaded: `lsmod | grep w1` shows `w1_gpio` and `wire` modules ✓

- [x] Additional configuration required
  - [x] Loaded `i2c_dev` kernel module: `sudo modprobe i2c_dev`
  - [x] Configure i2c-dev to load on boot: `echo 'i2c-dev' | sudo tee /etc/modules-load.d/i2c.conf`
...
### Completion Notes
**PART 1 COMPLETE - Infrastructure Files Created (commit: e0ed926):**

All repository infrastructure files have been created and committed:

1. **enable-interfaces.sh** - Idempotent automation script
2. **verify-interfaces.sh** - Comprehensive verification script with 7 tests
3. **config.txt.snippet** - Documented boot config template
4. **SETUP-GUIDE.md** - Updated with comprehensive Story 1.3 section

**PART 2 COMPLETE - Raspberry Pi Configuration Executed:**

Successfully deployed and executed on Raspberry Pi (opengardenlab.local):

1. **Scripts Deployed** - All setup scripts copied to `~/setup/` on Pi via SCP

2. **Automated Configuration** - Ran `enable-interfaces.sh` which:
   - Detected boot config at `/boot/firmware/config.txt` (newer Pi OS)
   - Added `dtparam=i2c_arm=on` for I2C interface
   - Added `dtoverlay=w1-gpio` for 1-Wire interface
   - Installed i2c-tools package (version 4.4)

3. **System Reboot** - Rebooted Pi to apply kernel module changes

4. **Additional Configuration** - Discovered and resolved `i2c_dev` module requirement:
   - Loaded `i2c_dev` module via `modprobe i2c_dev`
   - Created `/etc/modules-load.d/i2c.conf` to auto-load on boot

5. **Verification** - All 5 Acceptance Criteria PASSED:
   - ✅ AC1: `/dev/i2c-1` device node exists
   - ✅ AC2: `/sys/bus/w1/devices/` directory exists
   - ✅ AC3: i2c-tools installed (v4.4)
   - ✅ AC4: `i2cdetect -y 1` runs successfully (empty grid - no sensors yet)
   - ✅ AC5: `dtoverlay=w1-gpio` verified in boot config

**Note:** i2c-tools binaries installed to `/usr/sbin/` (full path required: `/usr/sbin/i2cdetect`)
